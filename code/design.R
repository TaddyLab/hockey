...
SHOTS = FALSE ## whether we count missed shots as well as goals
...
game.goals <- function(file, session)
  {
        ...
        ## get goals
        if(SHOTS){
          goals <- game[which(game$etype == "GOAL" | game$etype == "SHOT"),]
        }else{
...
clusterExport(cl,c("SHOTS","game.goals"))
...
## save
if(SHOTS){
  fname <- "data/nhldesign-shots.rda"
} else{
