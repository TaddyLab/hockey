...
configurations is relatively small.  Standard regression algorithms, such as
maximum likelihood inference via Fisher scoring, will either massively \textit
{over-fit} (e.g. assign large effects to players who rarely play) or simply
fail to converge.
...
A better measure of performance would be the partial effect of each player,
...
of these metrics offer a partial effect: .  An exception is the method of
\cite{ThoVenJen12}, which proposes a proportional hazards process model for
game events, allowing partial player effects to be backed out from high
resolution game data. A downside however is that the apparatus is a bit of a
black box to non-expert, and relies heavily on model assumptions that may be
difficult to validate in practice.
...
This chapter centers on the simple regression-based framework suggested by
